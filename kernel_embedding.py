...
def compute_mmd_score(etf_returns, reference_returns):
    # Add tiny noise to avoid degenerate cases (zero variance)
    eps = 1e-9
    if np.std(etf_returns) < eps:
        etf_returns = etf_returns + np.random.normal(0, eps, len(etf_returns))
    if np.std(reference_returns) < eps:
        reference_returns = reference_returns + np.random.normal(0, eps, len(reference_returns))
    X = etf_returns.reshape(-1, 1)
    Y = reference_returns.reshape(-1, 1)
    mmd_val = mmd_unbiased(X, Y)
    # If still zero (e.g., identical distributions), use a small positive based on mean difference
    if mmd_val == 0.0:
        mmd_val = abs(np.mean(etf_returns) - np.mean(reference_returns)) + eps
    return -mmd_val
